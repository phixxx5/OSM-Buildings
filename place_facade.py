...
class PlaceFacade():
    ...
    def Activated(self):
        ...
        # todo determine which point is which
        ...
        depth_l1 = self.fassade.corners[4]
        depth_l2 = self.fassade.corners[5]
        depth_r1 = self.fassade.corners[6]
        depth_r2 = self.fassade.corners[7]
        ...
        img_path = "facade_transformed.jpg"
        cv.imwrite(img_path, transformed_img)

        cv.imshow("window", img)

        wait_n_key()

        van_point_hor = self.line_line_intersection(top_left, top_right, bot_right, bot_left)
        van_point_ver = self.line_line_intersection(top_left, bot_left, top_right, bot_right)
        van_point_dep = self.line_line_intersection(depth_l1, depth_l2, depth_r1, depth_r2)
        ...
        perpendicular_h = self.perpendicular_through_point(van_point_hor, van_point_ver, van_point_dep)
        cv.line(img, van_point_hor, perpendicular_h, (0, 255, 255), 3)
        perpendicular_v = self.perpendicular_through_point(van_point_ver, van_point_dep, van_point_hor)
        cv.line(img, van_point_ver, perpendicular_v, (0, 255, 255), 3)
        perpendicular_d = self.perpendicular_through_point(van_point_dep, van_point_hor, van_point_ver)
        cv.line(img, van_point_dep, perpendicular_d, (0, 255, 255), 3)
        cv.imshow("window", img)
        wait_n_key()
        # compute circles?
        ortho_center = self.line_line_intersection(van_point_hor, perpendicular_h, van_point_ver, perpendicular_v)
        ...
        place_image(doc, img_path, clicked_face)
        doc.recompute()
        return
    ...
    def perpendicular_through_point(self, ortho_point_1, line_point_1, line_point_2):
        g1 = line_point_2[0] - line_point_1[0], line_point_2[1] - line_point_1[1]
        g2 = g1[1], -g1[0]
        ortho_point_2 = ortho_point_1[0] + g2[0], ortho_point_1[1] + g2[1]
        return self.line_line_intersection(ortho_point_1, ortho_point_2, line_point_1, line_point_2)
